...
def cozy_observation(**overrides):
    obs = {
        "invite": 1, "opt_in": 1, "text_reply": 1, "reaction": 1,
        "state_share": 0, "proactive_update": 0, "routine_maintenance": 1,
        "pass_event": 0, "resume_signal": 0, "tone_warmth": 0.55,
...
def test_posterior_probability_cup_is_filled_to_one():
    posterior = CoffeeParticleFilter(particle_count=400, seed=1234).update(cozy_observation())
...
def test_ess_stays_finite_positive_and_inside_the_flock():
    particle_count = 500
    posterior = CoffeeParticleFilter(particle_count=particle_count, seed=4321).update(cozy_observation())
...
def test_resampling_never_loses_any_tiny_particle_friends():
    ...
    for i in range(12):
        pf.update(cozy_observation(opt_in=i % 2, pass_event=(i + 1) % 2, reaction=i % 2, response_delay_min=5.0 + i * 20.0))
...
def test_missing_clues_are_allowed_to_stay_mysterious():
    posterior = CoffeeParticleFilter(particle_count=300, seed=77).update({"invite": 1, "opt_in": None, "reaction": None})
    assert np.all(np.isfinite(posterior.mean))
    assert np.isclose(posterior.mode_probabilities.sum(), 1.0)


def test_old_baskets_still_get_a_polite_little_legacy_hug():
    posterior = CoffeeParticleFilter(particle_count=300, seed=88).update({
        "cheng_invite": 1, "linda_opt_in": 1, "text_reply": 1, "reaction": 1,
        "state_share": 0, "proactive_update": 0, "routine_maintenance": 1,
        "pass_event": 0, "resume_signal": 0, "tone_warmth": 0.5,
