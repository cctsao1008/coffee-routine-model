...
from scenarios import DEFAULT_SCENARIO, SCENARIOS, get_scenario
from simulate import default_output_path, generate_truth, observe
...
def test_same_seed_grows_the_same_tiny_coffee_year():
    ...
    first_rng = np.random.default_rng(20260908)
    second_rng = np.random.default_rng(20260908)

    first_states, first_modes = generate_truth(40, first_rng, scenario)
    second_states, second_modes = generate_truth(40, second_rng, scenario)
...
def test_same_seed_makes_the_same_little_observation():
    ...
    first = observe(FIXED_STATE, RoutineMode.BUSY, first_rng, scenario)
    second = observe(FIXED_STATE, RoutineMode.BUSY, second_rng, scenario)
...
def test_every_mode_can_make_a_tiny_clue_without_falling_over():
    ...
    for mode in RoutineMode:
        rng = np.random.default_rng(100 + int(mode))
...
def test_every_weather_can_survive_a_small_particle_picnic():
    for i, scenario in enumerate(SCENARIOS.values()):
        ...
        for day in range(10):
            obs = observe(truth[day], RoutineMode(int(modes[day])), rng, scenario)
