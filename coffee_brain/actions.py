...
ACTION_EFFECTS = np.array(
    [
        ...
        [0.0060, 0.0040, 0.0000, 0.0020, 0.0000, -0.0030],  # A deliver
        [0.0040, 0.0010, 0.0010, 0.0000, 0.0000, -0.0040],  # A notify
        [0.0010, 0.0030, 0.0000, 0.0070, 0.0010, -0.0010],  # A callback
        [0.0000, 0.0010, 0.0060, 0.0000, 0.0000, -0.0050],  # A boundary preserving
        [0.0020, 0.0060, 0.0020, 0.0010, 0.0000, -0.0010],  # B opt in
        [0.0000, 0.0000, 0.0060, 0.0000, 0.0000, -0.0010],  # B voluntary pass
        [0.0010, 0.0050, 0.0000, 0.0020, 0.0000, -0.0020],  # B acknowledge
        [0.0030, 0.0030, 0.0040, 0.0020, 0.0000, -0.0060],  # B exception sync
        [0.0030, 0.0040, 0.0010, 0.0020, 0.0000, -0.0040],  # B closure
    ],
    dtype=float,
)
...
def action_effect(actions: RoutineActions | Mapping[str, float] | None) -> np.ndarray:
    ...
    voluntary choice, it can preserve V without silently reducing M. Cute boundary,
    serious semantics. ☕
    """
